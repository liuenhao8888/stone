...
public class BaseController {
	...
	protected TUser getCurrentUser(HttpServletRequest request) {
		String token = getCookie(request,CookieSet.tokenNname.toString());
	...
	public TUser getCurrentUser4redis(HttpServletRequest request) {
		String token = this.getCookie(request, "token");
		TUser u = (TUser) redisTemplate.opsForValue().get(token);
//		TUser u = (TUser) redisTemplate.opsForHash().get(SystemConstants.REDIS_USER, token);
		return u;
	}
